...
router.post("/phone_pin", async (req, res, next) => {
  try {
    let { err, account, state } = await directMobilePinLoginFunc(req.body);
    if (err) {
      res.status = 400;
      return res.json({ err });
    }
...
router.post("/next_token?", async (req, res, next) => {
  try {
    ...
    let next_factor =
      factor && unauthenticated_factors.find((fac) => fac === factor);
    ...
    if (!next_factor) {
      ...
      return res.json({
        err: { msg: "All factors verified", type: "full_ver" },
      });
    }
    ...
    switch (action) {
      ...
      case "activate_token": {
        let my_res = await activateNext({
          factorToActivate: next_factor,
          account,
        });
