...
router.use("/:verSessID", async (req, res, next) => {
  try {
    ...
    let allResponses = await retrieveAccountInfoByVerSessID(verSessID);
...
router.get("/:verSessID/next_factor", async (req, res, next) => {
  try {
    ...
    let next_factor = verInfo.status?.filter((obj) => obj.isVerified)[0]
      ?.factor;
...
router.get("/:verSessID/phone_pin", async (req, res, next) => {
  try {
    let { verInfo } = req.session;
    let emailStatus = verInfo.status.find(
      (item) => item.factor === "phone_pin"
    );
    res.json({ status: emailStatus });
  } catch (error) {
    console.log(error);
