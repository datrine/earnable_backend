...
const { sendEmail } = require("../utils/email_mgt");
...
const { getRandomToken } = require("../utils/token_mgt");
const { loginInfo } = require("./templates/account");
const { sendPhoneText } = require("../utils/phone_mgt");
/**
    * @param {object} params
    * @param {accTemplate} params.account
    * @param {string} params.factor
    * @param {boolean} params.factorVerSuccessful
    */
...
/**
    ...
    * @param {boolean} params.factorVerSuccessful
    */
async function accountAccIDResetSet({ email, sessID }) {
    try {
        /**
         * @type {accTemplate}
        */
...
/**
    * @param {object} params
    * @param {accTemplate} params.account
    * @param {"direct"|"mobile"|"email"} params.factor
    * @param {boolean} params.factorVerSuccessful
    */
async function accountSet({ account, factor, factorVerSuccessful = true }) {
    try {
        ...
        if (loginInfo.factors.indexOf("direct") === -1)
            loginInfo.factors = ["direct", "email", "mobile"]
        //check if other factors have been verified...

        loginInfo.current_session.factors_verified =
            loginInfo.current_session.factors_verified || []

        //check if the factor exists
        let factorExists =
            loginInfo.current_session.factors_verified.some((item, index) =>
                item.factor === factor);
        ...
        if (!factorExists && factorVerSuccessful)
            loginInfo.current_session.factors_verified.push({ factor, time: new Date() });

        if (!factorVerSuccessful) {
        ...
        let lengthOfVerified = loginInfo.current_session.factors_verified.length;
...
async function emailFactorDBUpdate(email) {
    try {
        ...
        if (!accountBeforeSetup) {
            console.log("Email token failed...")
            return { err: { msg: "Email token failed..." } };
        }

        //accountBeforeSetup.loginInfo.current_session.factors_verified.
        let response = await accountSet({
            account: accountBeforeSetup,
            factor: "email", factorVerSuccessful: true
        });
...
async function mobileFactorDBUpdate(phonenum) {
    try {
        ...
        if (!accountBeforeSetup) {
            console.log("Phone number token failed...")
            return { err: { msg: "Phone number token failed..." } };
        }

        //accountBeforeSetup.loginInfo.current_session.factors_verified.
        let response = await accountSet({
            account: accountBeforeSetup,
            factor: "mobile", factorVerSuccessful: true
        });
        let { err, state, account } = response;
        if (err)
            return { err }
...
async function directFactorDBUpdate(directCreds) {
    try {
        ...
        if (!matches) {
            ...
            let response = await accountSet({
                account: accountBeforeSetup,
                factor: "direct", factorVerSuccessful: false
            });
...
async function retrieveAccountInfoBySessID(sessID) {
    try {
        ...
        if (!account) {
            console.log("SessID does not exist in record...")
...
async function retrieveAccountInfoByVerSessID(verSessID) {
    try {
        ...
        if (!account) {
            console.log("verSessID does not exist in record...")
 ...
 * @param {"email"|"mobile"} param0.factor
...
async function activateNext({ factorToActivate, account }) {
    console.log(account)
    console.log("factorToActivate: " + factorToActivate)
    try {
...
let getUnAuthenticatedFactors = (account) => account.loginInfo.factors.filter(factor => {
    return !account.loginInfo.current_session.
        factors_verified.some(elem => elem.factor === factor)
});
...
module.exports = {
    ...
    retrieveAccountInfoByAccountID, deepSearchAccountsAndUsers
};