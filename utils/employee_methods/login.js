...
const { directFactorAuth, emailFactorAuth, mobileFactorAuth } = require("./account");

async function directLoginFunc(data) {
    try {
        let { err, account, state } = //await retrieveAccountInfoFunc({ identifier, password });
            await directFactorAuth(data)
        if (err) {
            return { err }
        }
...
module.exports = { directLoginFunc, emailLoginFunc, 
    retrieveAccountInfoFunc, mobileSMSLoginFunc};