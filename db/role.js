...
let getScopeByAccID = async ({ companyID, accountID }) => {
  try {
    let result1 = await companyRolesCol.findOne({
      companyID,
      roles: { $elemMatch: { "listOfUsers.accountID": accountID } },
    });
    if (!result1) {
      return { err: { msg: "No roles match companyID." } };
    }
...
let addUsersToRole = async ({ accounts = [], companyID, role }) => {
  try {
    ...
    let comp = await companyRolesCol.updateOne(
      { companyID },
      { $set: { ...rolesDoc.roles } }
    );
...
let acc = { ...accTemplate, rolenames: [""] };
/**
 *
 * @param {object} param0
 * @param {[acc]} param0.accountsRolesMap
 * @param {object} param0
 * @returns
 */
let addUsersToRoles = async ({ accountsRolesMap = [], companyID }) => {
  try {
    let rolesDoc = await companyRolesCol.findOne({
      companyID,
    });
    ...
    for (const { rolenames, ...account } of accountsRolesMap) {
      for (const rolename of rolenames) {
        let roleObj = rolesDoc.roles?.find((obj) => obj.name === rolename);
        if (!roleObj) {
          return { err: { msg: "Role doesn't not exist." } };
        }
        roleObj.listOfUsers.push(createRole(rolename, account));
      }
    }
    let comp = await companyRolesCol.updateOne(
      { companyID },
...
let removeUsersFromRole = async ({ accounts = [], companyID, role }) => {
  try {
    ...
    for (const account of accounts) {
      ...
      let indexOfAccUser = listOfUsers.findIndex(
...
let hasRole = async ({ accountID, rolename }) => {
  try {
    let rolesDoc = await companyRolesCol.findOne({
      roles: {
        $elemMatch: { name: rolename, "listOfUsers.accountID": accountID },
      },
    });
