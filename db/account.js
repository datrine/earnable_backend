...
async function accountLogOut({ sessID }) {
  try {
    ...
    if (!accountCheckd?.loginInfo?.current_session) {
      return {
        err: { msg: "No account matches sessID...", type: "no_acc" },
        state: "failed",
      };
    }
    ...
    if (sessExpiresOn <= new Date()) {
      return {
        err: { msg: "Session has already expired...", type: "sess_exp" },
        state: "sess_expired",
      };
    }
    let response = await accountsCol.findOneAndUpdate(
      ...
      },
      {
        $set: {
          ...
          "loginInfo.last_login": loginInfo.current_session.completion_time,
        },
      },
...
async function accountPhonePinSet({ account, factor, facAuthSuc = true }) {
  try {
    ...
    let factorExists = loginInfo.current_session.factors_authenticated.some(
      (item, index) => item.factor === factor
    );
    ...
    if (!factorExists && facAuthSuc)
      loginInfo.current_session.factors_authenticated.push({
        factor,
        time: new Date(),
      });
    ...
    let lengthOfVerified =
      loginInfo.current_session.factors_authenticated.length;
    ...
    loginInfo.current_session.updatedOn = new Date();
    loginInfo.current_session.sessID =
      loginInfo.current_session.sessID || nanoid();
    let response = await accountsCol.findOneAndUpdate(
      { _id: account._id },
      {
...
async function emailFactorAuth({ email, token }) {
  try {
    ...
    let { err, account, state } = await emailFactorDBUpdate(
      tokenDoc.factorValue
    );
    if (err) {
...
async function emailFactorDBUpdate(email) {
  try {
    ...
    if (err) return { err };
    if (state.value != "authenticated") return { state };
    return { account, state };
  } catch (error) {
    console.log(error);
    throw error;
...
async function mobileFactorAuth({ email, token }) {
  try {
    ...
    let { err, account, state } = await mobileFactorDBUpdate(
      tokenDoc.factorValue
    );
    if (err) {
      return { err, state: "failed" };
...
async function setDefaultPhonePin({ phonenum, phonePin }) {
  try {
    ...
    let accSetRes = await accountPhonePinSet({
      account: acc,
      factor: "phone_pin",
      facAuthSuc: false,
    });
...
async function directPhonePinAuth({ phonenum, pin }) {
  try {
    ...
    let { err, account, state } = await phonePinLoginDBUpdate({
      phonenum,
      pin,
    });
    if (err) return { err };
...
async function directFactorAuth({ identifier, password }) {
  try {
    ...
    let { err, account, state } = await directFactorDBUpdate({
      identifier,
      password,
    });
    if (err) return { err };
...
async function activateEmployeeAccount({ identifier, verSessID, phonePin }) {
  try {
    ...
    if (account?.verInfo?.verSessID !== verSessID) {
      return {
        err: { msg: "Account verification session ID does not match record." },
      };
    }
...
async function updateAccVer({
  ...
  try {
    ...
    let { verInfo } = account;
    //find factor
    let verFactorObj = verInfo.status.find((obj) => obj.factor === factor);
    if (verFactorObj) {
      ...
      verFactorObj.timeVerified = verFactorObj.isVerified ? new Date() : null;
    }
 ...
 * @returns
 */
async function updateAccInfo({ accountID, prop, propValue }) {
  try {
    /**
     * @type {accTemplate}
     */
    ...
    if (!account) {
      return { err: { msg: "account does not exist in record..." } };
    }
    ...
    let { ok, lastErrorObject, value } = updateRes;
    return { info: "Account info updated..." };
  } catch (error) {
    console.log(error);
  }
}
 ...
 * @returns
 */
let getUnAuthenticatedFactors = (account) =>
  account.loginInfo.factors.filter((factor) => {
    return !account.loginInfo?.current_session?.factors_authenticated?.some(
      (elem) => elem.factor === factor
    );
  });
 ...
 * @returns
 */
let getUnverifiedFactors = (account) =>
  account.verInfo.status.filter((factor) => !factor.isVerified);
...
module.exports = {
  ...
  activateNext,
  updateAccVer,
  getUnAuthenticatedFactors,
  accountLogOut,
  retrieveAccountInfoByAccountID,
  deepSearchAccountsAndUsers,
  changePhonePin,
  setDefaultPhonePin,
